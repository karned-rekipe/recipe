...
async def test_find_by_name_match(repo):
    await repo.create(Ingredient(name = "Farine de blé", unit = None))
    await repo.create(Ingredient(name = "Sel fin", unit = None))
...
async def test_find_by_name_case_insensitive(repo):
    await repo.create(Ingredient(name = "Farine", unit = None))
    assert len(await repo.find_by_name("FARINE")) == 1


async def test_find_by_name_includes_deleted(repo):
    deleted = Ingredient(name = "Farine", unit = None, deleted_at = datetime.now(timezone.utc))
...
async def test_find_all_excludes_deleted(repo):
    await repo.create(Ingredient(name = "Farine", unit = None))
    deleted = Ingredient(name = "Sel", unit = None, deleted_at = datetime.now(timezone.utc))
    await repo.create(deleted)
...
async def test_duplicate(repo):
    i = await repo.create(Ingredient(name = "Farine", unit = None))
...
async def test_duplicate_deleted_raises(repo):
    i = Ingredient(name = "Farine", unit = None, deleted_at = datetime.now(timezone.utc))
    await repo.create(i)
